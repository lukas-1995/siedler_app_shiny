...
function(input,output, session){
  ...
  observeEvent(input$run,{
    ...
    player_names <- loadData("player_names")
  ...
  observeEvent(input$change,{session$reload()
    # {
    #   user <- sqlQuery(con, "SELECT * FROM user")
    # names(user) <- "Users"
    # waits$user <- 0
  ...
  observeEvent(input$newplconf,{
    ...
    saveData(player_names, "player_names")
